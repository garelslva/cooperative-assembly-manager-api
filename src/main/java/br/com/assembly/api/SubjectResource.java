...
@Tags(value = {
        @Tag( name = "(Pautas) Subject Api", description = "Api to subject management")
})
@Slf4j
@RestController
...
public class SubjectResource {
    ...
    @GetMapping
    public ResponseEntity<PageDTO<SubjectDTO>> getAll(@Valid @Parameter(hidden = true) final PageableDTO pageable) {
        log.info("Start listener all subjects");
        return ResponseEntity.ok(pageConverter.toPageDto(this.subjectConverter.toDto(
                subjectService.findAll(this.pageableConverter.toDomain(pageable)))));
    }

    @Operation(summary = "Find all subjects records by id")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = MessageError.SUCCESS, content = { @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = SubjectDTO.class))}),
            @ApiResponse(responseCode = "400", description = MessageError.ERROR_BAD_REQUEST, content = {@Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = ResponsePayloadError.class))}),
            @ApiResponse(responseCode = "404", description = MessageError.NOT_FOUND, content = {@Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = ResponsePayloadError.class))}),
            @ApiResponse(responseCode = "500", description = MessageError.UNEXPECTED_ERRO, content = {@Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = ResponsePayloadError.class))})})
    @Parameter(name ="id", in = ParameterIn.PATH, schema = @Schema(description = "id", type = "Long", implementation = Long.class), required = true)
    @GetMapping("/{id}")
    public ResponseEntity<SubjectDTO> getById(@Valid @NotNull @PathVariable("id") final Long id) {
        log.info("Start listener subject to id: {}", id);
        return ResponseEntity.ok(this.subjectConverter.toDto(
                subjectService.findById(id)));
    }

    @Operation(summary = "Save record one subjects")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = MessageError.SUCCESS, content = { @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = SubjectDTO.class))}),
            @ApiResponse(responseCode = "400", description = MessageError.ERROR_BAD_REQUEST, content = {@Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = ResponsePayloadError.class))}),
            @ApiResponse(responseCode = "500", description = MessageError.UNEXPECTED_ERRO, content = {@Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = ResponsePayloadError.class))})})
    @PostMapping
    public ResponseEntity<SubjectDTO> save(@Valid @NotEmpty @RequestBody final SubjectDTO subject) {
        log.info("Start save subject");
        return ResponseEntity.status(HttpStatus.CREATED).body(this.subjectConverter.toDto(
                subjectService.save(this.subjectConverter.toDomain(subject))));
    }

    @Operation(summary = "Delete subjects records by id")
    @ApiResponses(value = {
    ...
    public ResponseEntity deleteById(@Valid @NotNull @PathVariable("id") final Long id) {
        log.info("Start delete subject to id: {}", id);
